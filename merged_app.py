...
BKASH_BASE_URL = "https://checkout.pay.bka.sh/v1.2.0-beta"  # use sandbox URL first
...
@app.route('/pay-salary/<int:employee_id>', methods=['POST'])
def pay_salary(employee_id):
    ...
    # Optional: create a notification table entry here
    db.session.commit()
    flash(f'Salary paid to {employee.name}', 'success')
    return redirect(url_for('admin_dashboard'))

@app.route('/pay-salary/<int:employee_id>', methods=['GET'])
def pay_salary_form(employee_id):
    employee = EmployeeProfile.query.get_or_404(employee_id)
    payable = calculate_salary(employee)
    return render_template('pay_salary.html', employee=employee, payable=payable)

@app.route('/pay-salary/<int:employee_id>', methods=['POST'])
@app.route('/pay-salary/<int:employee_id>', methods=['POST'])
@app.route('/pay-salary/<int:employee_id>', methods=['POST'])
def pay_salary_submit(employee_id):
    employee = EmployeeProfile.query.get_or_404(employee_id)
    transaction_id = request.form['transaction_id']
    admin_pin = request.form['admin_pin']  # Get the entered admin pin

    # Verify the admin pin
    if admin_pin != ADMIN_PIN:
        flash("❌ Incorrect admin pin. Payment not processed.", "danger")
        return redirect(url_for('pay_salary_form', employee_id=employee.id))  # Return to the form

    # Proceed with payment verification via Bkash API
    try:
        ...
        verify_url = f"{BKASH_BASE_URL}/payment/status"
        response = requests.post(verify_url, json={"paymentID": transaction_id}, headers=headers)
        response_data = response.json()

        if response_data.get("transactionStatus") == "Completed":
            employee.salary_paid = True
            db.session.commit()
            flash(f"✅ Payment verified for {employee.name}", "success")
        else:
            flash(f"❌ Payment verification failed: {response_data.get('transactionStatus')}", "danger")
    except Exception as e:
