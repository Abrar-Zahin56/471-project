...
# Routesfrom flask_sqlalchemy import SQLAlchemy
from werkzeug.security import generate_password_hash, check_password_hash
from datetime import datetime
import os
from flask import jsonify
...
def admin_dashboard():
    ...
    active_emergencies = Emergency.query.filter(Emergency.status != 'resolved').all()
    ...
    return render_template(
        ...
        personnel=employees  # or use `personnel` if that's semantically different
    )
@app.route('/employee')
...
def resolve_emergency(emergency_id):
    ...
    profile = EmployeeProfile.query.filter_by(user_id=session.get('user_id')).first()
    if profile:
        profile.status = 'available'  # or 'active' if you prefer keeping them active post-resolution
